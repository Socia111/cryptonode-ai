-- Phase 4: Fix Database Permissions & RLS Policies for Live Trading System
...
-- Fix exchange_feed_status access
...
-- Add index for better signal performance
CREATE INDEX IF NOT EXISTS idx_signals_realtime 
ON signals (created_at DESC, source, is_active) 
WHERE expires_at > now() AND source LIKE '%real%';
