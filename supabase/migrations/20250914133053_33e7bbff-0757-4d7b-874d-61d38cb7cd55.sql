-- Complete Supabase Database Rebuild (Vector-Safe)
-- This migration drops everything and rebuilds from scratch, avoiding vector extension conflicts
...
-- Drop all existing functions and procedures (excluding vector and system functions)
DO $$
DECLARE
    func_record RECORD;
BEGIN
    FOR func_record IN 
        ...
        AND p.proname NOT LIKE 'halfvec_%'
        AND p.proname NOT LIKE 'sparsevec_%'
        AND p.proname NOT LIKE 'array_to_vector'
        AND p.proname NOT LIKE 'array_to_halfvec'
        AND p.proname NOT LIKE 'array_to_sparsevec'
    LOOP
        BEGIN
...
-- Drop all custom types (excluding vector types)
DO $$
DECLARE
    type_record RECORD;
BEGIN
    FOR type_record IN 
        ...
        AND t.typname NOT LIKE 'vector'
        AND t.typname NOT LIKE 'halfvec'
        AND t.typname NOT LIKE 'sparsevec'
    LOOP
