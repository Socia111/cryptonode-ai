...
-- Auto-trading configuration per user (if multi-user)
...
CREATE TRIGGER trigger_trading_configs_updated_at
...
CREATE POLICY "Service role full access on trading_configs" ON public.trading_configs
  FOR ALL USING (auth.role() = 'service_role');

-- Users can view their own data (if multi-user in future)
...
CREATE POLICY "Users can manage their trading config" ON public.trading_configs
